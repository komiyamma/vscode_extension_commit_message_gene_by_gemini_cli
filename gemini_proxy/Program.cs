...
class Program
{
    ...
    // Ctrl イベント時の安全なクリーンアップ用の静的参照
    private static volatile Process s_childProcess;
    private static KillOnCloseJob s_jobRef;
    private static int s_cleanupDone; // 0: 未実行, 1: 実行済み
    ...
    // GC で回収されないよう static フィールドに保持
    private static readonly ConsoleCtrlDelegate s_ctrlHandler = new ConsoleCtrlDelegate(ConsoleCtrlHandler);

    private static bool ConsoleCtrlHandler(int ctrlType)
    {
        // 0:CTRL_C, 1:CTRL_BREAK は Console.CancelKeyPress へ委譲
        ...
        {
            return false; // 既存の CancelKeyPress に任せる
        }

        if (ctrlType == CTRL_CLOSE_EVENT || ctrlType == CTRL_LOGOFF_EVENT || ctrlType == CTRL_SHUTDOWN_EVENT)
        {
            if (Interlocked.Exchange(ref s_cleanupDone, 1) == 0)
            {
                try
                {
                    var p = s_childProcess;
                    if (p != null)
                    {
    ...
    {
        // Ctrl ハンドラの登録（早期）
        try { SetConsoleCtrlHandler(s_ctrlHandler, true); } catch { }
        ...
        try
        {
            ...
            s_jobRef = job; // Ctrl イベントから参照できるよう保持
            ...
            s_childProcess = process; // Ctrl イベントから参照できるよう保持
                ...
                try
                {
                    ...
                    try
                    {
                        if (process != null && !process.HasExited)
                        {
                            ...
                            process.Dispose();
                            if (job != null) job.Dispose(); // CloseHandle -> 子プロセス一括終了
                        }
                    }
                    catch { /* 例外は無視 */ }
                }
